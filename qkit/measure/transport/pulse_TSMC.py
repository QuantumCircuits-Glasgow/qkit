...
class transport(object):
    ...
    def measure_transistor_IDS_vs_VDS(self,SMU_VDS,value_VGS,value_VDS,file_name_m):
        linestyle_tuple = [
        ('solid',                 (0, ())),
        ('dashed',                (0, (5, 5))),
        ('dotted',                (0, (1, 1))),
        ...
        headers = ["VDS","VGS","IDS"]
        f=IntProgress(min=0,max=len(value_VGS)*len(value_VDS))
        display(f)
        ...
        file_name = os.path.join(qkit.cfg['datadir'], file_name_m +".csv")
        


        with open(file_name,"w",newline="") as csvfile :
            logfile = csv.writer(csvfile)
            logfile.writerow(["VDS","VGS","IDS","IGS"])

            for g in range (0,len(value_VGS),1) : ##### y(3,16,2)
                self._IVD._visainstrument.write("SOUR:VOLT "+str(value_VGS[g]))
                current1 = self._IVD._visainstrument.query_ascii_values("MEAS:CURR?")
                for d in range (0,len(value_VDS),1) : #### x(0,19,1)
                    SMU_VDS._visainstrument.write("SOUR:VOLT "+str(value_VDS[d]))
                    current2 = SMU_VDS._visainstrument.query_ascii_values("MEAS:CURR?")
                    SMU_VDS._visainstrument.query("*OPC?")
                    time.sleep(0.1)
                    f.value += 1
                    current_IDS = SMU_VDS._visainstrument.query_ascii_values("READ?")
                    current_IGS = self._IVD._visainstrument.query_ascii_values("READ?")
                    logfile.writerow([value_VDS[d],value_VGS[g],current_IDS[0],current_IGS[0]])
        ...
        time.sleep(1) 
        #SMU_VDS._visainstrument.write(":OUTP:STAT OFF") # Turn OFF the Output of SMU
        #self._visainstrument.write(":OUTP:STAT OFF") # Turn OFF the Output of SMU
        #self._IVD._visainstrument.write(":OUTP:STAT OFF") # Turn OFF the Output of SMU

        #self._IVD._visainstrument.close()
        #self._visainstrument.close()
        #SMU_VDS._visainstrument.close()
        plt.rcParams["figure.figsize"] = [15, 10]
        plt.rcParams["figure.autolayout"] = True

        headers = ["VDS","VGS","IDS","IGS"]
        ...
        for g in range (0,len(value_VGS),1) : 
            plt.plot(df.VDS[i*(len(value_VDS)):(i+1)*(len(value_VDS))], df.IDS[i*(len(value_VDS)):(i+1)*(len(value_VDS))]*1e6, label='VGS = '+str(df.VGS[g*len(value_VDS)])+'V'
                     ,color="rbgkmcyrbgkmcy"[g],linestyle=linestyle_tuple[0][1])
            i = i + 1
        


        plt.title(file_name_m)
        ...
        plt.savefig(file_name.replace("csv","png"),dpi=200)

        plt.show()

    def measure_transistor_dualsweep_IDS_vs_VDS(self,SMU_VDS,value_VGS,value_VDS,file_name_m):
        linestyle_tuple = [
        ('solid',                 (0, ())),
        ('dashed',                (0, (5, 5))),
        ('dotted',                (0, (1, 1))),
        ...
        headers = ["VDS","VGS","IDS"]
        f=IntProgress(min=0,max=len(value_VGS)*len(value_VDS)*2)
        display(f)
        current_datetime = datetime.now()
        print("Measurement Start time : ", current_datetime)
        ...
        file_name = os.path.join(qkit.cfg['datadir'], file_name_m +".csv")
        print(file_name)
        value_VDS_reversed=value_VDS[::-1]
        value_VDS.extend(value_VDS_reversed)
        print(value_VDS)
    ...
    def measure_transistor_IDS_vs_VGS(self,SMU_VDS,value_VGS,value_VDS,file_name_m):
        linestyle_tuple = [
        ('solid',                 (0, ())),
        ('dashed',                (0, (5, 5))),
        ('dotted',                (0, (1, 1))),
        ...
        with open(file_name,"w",newline="") as csvfile :
            ...
            for d in range (0,len(value_VDS),1) : ##### y(3,16,2)
                SMU_VDS._visainstrument.write("SOUR:VOLT "+str(value_VDS[d]))
                current1 = SMU_VDS._visainstrument.query_ascii_values("MEAS:CURR?")
                for g in range (0,len(value_VGS),1) : #### x(0,19,1)
                    self._IVD._visainstrument.write("SOUR:VOLT "+str(value_VGS[g]))
                    current2 = self._IVD._visainstrument.query_ascii_values("MEAS:CURR?")
                    time.sleep(0.1)
                    f.value += 1
                    current_IDS = SMU_VDS._visainstrument.query_ascii_values("READ?")
                    current_IGS = self._IVD._visainstrument.query_ascii_values("READ?")
        ...
        i = 0
        for d in range (0,len(value_VDS),1) : ##### y(3,16,2)
            plt.plot(df.VGS[i*(len(value_VGS)):(i+1)*(len(value_VGS))], df.IDS[i*(len(value_VGS)):(i+1)*(len(value_VGS))]*1e6, label='VDS = '+str(df.VDS[d*len(value_VGS)])+' V '
                     ,color="rbgkmcy"[d],linestyle=linestyle_tuple[0][1])
            i = i + 1
        #print(d)
        plt.title(file_name_m)
        plt.ylabel('$I_{SD}$ ($\mu$A)')
    ...
    def measure_transistor_dualsweep_IDS_vs_VGS(self,SMU_VDS,value_VGS,value_VDS,file_name_m):
        linestyle_tuple = [
        ('solid',                 (0, ())),
        ('dashed',                (0, (5, 5))),
        ('dotted',                (0, (1, 1))),
        ...
        value_VGS_reversed=value_VGS[::-1]
        value_VGS.extend(value_VGS_reversed)
        print(value_VGS)
