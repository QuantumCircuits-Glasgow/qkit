...
class circuit:
    ...
    def autofit(self, calc_errors=True, fixed_delay=None, fixed_phi=None, isolation=15):
        """
    ...
    def _extract_Qs(self, refine_results=False, calc_errors=True):
        ...
        #self.Qc = self.absQc * np.cos(self.phi)
       # self.Qi = 1. / (1./self.Ql - np.cos(self.phi)/self.absQc)
    ...
    def plotall(self,trans=0.5):
        ...
        plt.scatter(real,imag,label='rawdata',s=1,c='C1')     #TC2024, added scatter
        plt.plot(real2,imag2,label='fit',alpha=trans)
        plt.xlabel('Re(S21)')
        plt.ylabel('Im(S21)')
        plt.legend()
        plt.subplot(222)
        plt.scatter(self.f_data*1e-9,np.absolute(self.z_data_raw),label='rawdata',s=1,c='C1')
        plt.plot(self.f_data*1e-9,np.absolute(self.z_data_sim),label='fit',alpha=trans)
        plt.xlabel('f (GHz)')
        plt.ylabel('|S21|')
        plt.legend()
        plt.subplot(223)
        plt.scatter(self.f_data*1e-9,np.angle(self.z_data_raw),label='rawdata',s=1,c='C1')
        plt.plot(self.f_data*1e-9,np.angle(self.z_data_sim),label='fit',alpha=trans)
        plt.xlabel('f (GHz)')
        plt.ylabel('arg(|S21|)')
        plt.legend()
        plt.subplot(224)
        ff= ( f'Delay = {self.delay} \n' 
            f'Impedance = {self.phi} \n' 
            f'Delay left = {self.delay_remaining} \n' 
            f'fr = {round(self.fr*10**-9,4)} GHz  \n' 
            f'Qi = {round(self.Qi,1)} \n' 
            f'Qc = {round(self.Qc,1)} \n' 
            f'QL = {round(self.Ql,1)} \n' 
            rf'Qi_rel_error = $ \frac{{{round(self.Qi_err,2)}}}{{{round(self.Qi,2)}}} $ ={round(self.Qi_err/self.Qi,4)}' )
        plt.text(0.2,0.1,ff)
        #plt.show()  #TC2024, removing show to make edits
    ...
    def plotrawdata(self):
        ...
        imag = self.z_data_raw.imag
        plt.subplot(221)
        plt.scatter(real,imag,label='rawdata',s=1)
        plt.xlabel('Re(S21)')
        plt.ylabel('Im(S21)')
        plt.legend()
        plt.subplot(222)
        plt.scatter(self.f_data*1e-9,np.absolute(self.z_data_raw),label='rawdata',s=1)
        plt.xlabel('f (GHz)')
        plt.ylabel('|S21|')
        plt.legend()
        plt.subplot(223)
        plt.scatter(self.f_data*1e-9,np.angle(self.z_data_raw),label='rawdata',s=1)
        plt.xlabel('f (GHz)')
        plt.ylabel('arg(|S21|)')
        plt.legend()
        plt.show()
